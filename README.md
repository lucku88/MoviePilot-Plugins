...
| 2 | [✅ 自用签到 (PrivateCheckin)](#2--自用签到-privatecheckin) | v0.3.0 | 支持自定义浏览器签到页与 GET/POST 接口签到，浏览器型任务默认优先 Playwright 过 CF | 站点 |
...
- 版本：v0.3.0
- 功能：支持自定义浏览器签到页与 GET/POST 接口签到，浏览器型任务默认优先使用 Playwright 过 Cloudflare。
- 标签：站点
- 特点：
  ...
  - ➕ 默认不预置任务，按你的站点实际情况手动新增即可
- 更新说明：
  <details>
  <summary>点击查看更新历史</summary>

  - v0.3.0: 移除新建任务里的旧专用模式，下拉只保留通用签到方式；默认任务改为空，并重新调整 Vue 页面宽度和边距。
