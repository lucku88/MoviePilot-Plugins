...
| 2 | [✅ 自用签到 (PrivateCheckin)](#2--自用签到-privatecheckin) | v0.1.0 | 支持自定义签到页、思齐签到/HNR 领取、New API 签到，以及 Playwright/FlareSolverr CF 兜底 | 站点 |
...
- 版本：v0.1.0
- 功能：支持自定义签到页、思齐签到/HNR 领取、New API 签到，以及 Playwright/FlareSolverr 的 Cloudflare 兜底。
- 标签：站点
- 特点：
  - ✅ 提供 Vue 数据页和配置页，可直接在插件内维护任务列表
  - 🌐 通用任务支持自定义签到页地址，适合 `attendance.php` 这类“浏览器打开即自动签到”的站点
  - 🧠 内置 `思齐签到`、`思齐 HNR 领取`、`New API 签到` 三类特殊逻辑
  - 🍪 支持优先读取 MoviePilot 站点 Cookie，也支持为非 PT 站点手动填写浏览器 Cookie
  - 🛡️ 支持 `请求 -> Playwright -> FlareSolverr` 的分层 CF 兜底策略
  - 📋 默认带上 `ourbits / hddolby / ubits / audiences / 思齐 / New API` 示例任务，可直接改
- 更新说明：
  <details>
  <summary>点击查看更新历史</summary>
