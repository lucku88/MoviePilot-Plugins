...
**架构：** 保留现有 requests Session、超时和单层重试逻辑，删除持续强制 IPv4 的全局改写。配置迁移采用“读取时忽略、保存时丢弃”方式；热更新时仅识别旧版留下的特定 IPv4 lambda 并一次性恢复系统地址族选择，无需重置配置或重启 MoviePilot。
...
- 修改 `plugins.v2/vueemoji/__init__.py`：删除 IPv4 状态、配置字段和持续全局网络改写，并补充旧版热更新残留的窄范围清理。
...
新增测试，要求源码和配置不再包含 IPv4 强制逻辑，并覆盖热更新迁移：
...
def test_hot_upgrade_restores_legacy_ipv4_selector_to_system_default(self):
    # 模拟旧版留下的 IPv4 lambda，断言恢复为系统双栈选择。
    ...
...
预期：测试因仍存在 `force_ipv4`，且缺少热更新清理方法而失败。
...
# 删除每次创建 Session 时强制写入 IPv4 lambda 的逻辑
# 仅在 init_plugin 初始化时识别来源确属 Vue-表情旧版的特定 lambda
# 一次性恢复系统地址族选择，不在每次请求前改写全局状态
