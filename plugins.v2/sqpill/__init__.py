...
class SQPill(_PluginBase):
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [
            {"path": "/config", "endpoint": self._get_config, "methods": ["GET"], "auth": "bear", "summary": "获取 SQ魔丸配置"},
            {"path": "/config", "endpoint": self._save_config, "methods": ["POST"], "auth": "bear", "summary": "保存 SQ魔丸配置"},
            {"path": "/status", "endpoint": self._get_status, "methods": ["GET"], "auth": "bear", "summary": "获取 SQ魔丸状态"},
            {"path": "/refresh", "endpoint": self._refresh_data, "methods": ["POST"], "auth": "bear", "summary": "刷新 SQ魔丸状态"},
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._enabled:
            ...
            if next_run:
                services.append({
                    "id": "SQPill_auto",
                    "name": "SQ魔丸初始化" if self._bootstrap_pending else "SQ魔丸智能调度",
                    "trigger": "date",
    ...
    def _refresh_data(self):
        try:
            ...
            return {"success": True, "message": "SQ魔丸状态已刷新", "pill_status": status, "status": self._build_status(auto_refresh=False)}
        except Exception as err:
            detail = self._get_error_detail(err)
