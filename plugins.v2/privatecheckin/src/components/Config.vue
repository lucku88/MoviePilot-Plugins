    ...
    <div class="pc-shell">
      <section class="pc-hero">
        <div class="pc-hero-copy">
          ...
          <p class="pc-subtitle">
            这里统一管理签到页、接口请求、Cookie 和过 CF 策略。浏览器型签到默认优先用 MoviePilot 自带的 Playwright，
            接口型签到则按 requests 执行。
          </p>
        </div>
      ...
      <section class="pc-grid">
        ...
        <article class="pc-panel">
          ...
          <div class="pc-note-stack">
            <div class="pc-note">
              `智能兜底` 会按 `Playwright -> requests -> FlareSolverr` 顺序尝试，浏览器型签到默认更推荐 `仅 Playwright`。
            </div>
            <div class="pc-note">
              如果站点已经配置在 MoviePilot 站点管理里，可以打开“优先使用 MoviePilot Cookie”；否则直接填写浏览器 Cookie。
            </div>
          </div>
        </article>
      </section>
      ...
      <section class="pc-panel">
        ...
        <v-alert type="info" variant="tonal" class="mb-4">
          默认示例只提供填写模板，不会预设真实站点地址。把示例里的站点地址、任务地址、请求体、Cookie 改成你自己的再启用即可。
        </v-alert>
              ...
              <div class="pc-task-layout">
                ...
                <section class="pc-task-group">
                  <div class="pc-group-head">
                    ...
                    <div class="pc-mini">浏览器型签到默认建议 Playwright，接口型签到默认建议 requests。</div>
                  </div>
                ...
                <section v-if="usesRequestPayload(task.task_type)" class="pc-task-group">
                  <div class="pc-group-head">
                    ...
                    <div class="pc-mini">接口型签到支持自定义请求头和请求体，便于适配 GET / 表单 / JSON 这几类常见方式。</div>
                  </div>
