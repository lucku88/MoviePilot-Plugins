...
from app.plugins.fnmediacovergenerator.utils.image_manager import (
    ResolutionConfig, ImageResourceManager, managed_image, managed_images
)
from app.plugins.fnmediacovergenerator.utils.performance_helper import (
...
def create_style_static_1(image_path, title, font_path, font_size=(170,75), font_offset=(0,40,40), blur_size=50, color_ratio=0.8, resolution_config=None, bg_color_config=None):
    try:
        ...
        with managed_image(str(image_paths[0]), "RGB") as original_img, \
            ...
            # 使用动态字体大小
            zh_font, _ = load_font_with_fallback(zh_font_path, title_zh, int(zh_font_size))
            en_font, _ = load_font_with_fallback(en_font_path, title_en, int(en_font_size))
