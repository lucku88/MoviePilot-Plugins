import { importShared } from './__federation_fn_import-b37dd681.js';
import { _ as _export_sfc } from './_plugin-vue_export-helper-c4c0bc37.js';

const Page_vue_vue_type_style_index_0_scoped_7f0e16e0_lang = '';

const {createElementVNode:_createElementVNode,toDisplayString:_toDisplayString,createTextVNode:_createTextVNode,resolveComponent:_resolveComponent,withCtx:_withCtx,createVNode:_createVNode,openBlock:_openBlock,createBlock:_createBlock,createCommentVNode:_createCommentVNode,renderList:_renderList,Fragment:_Fragment,createElementBlock:_createElementBlock,vModelText:_vModelText,withDirectives:_withDirectives,normalizeClass:_normalizeClass,normalizeStyle:_normalizeStyle,pushScopeId:_pushScopeId,popScopeId:_popScopeId} = await importShared('vue');


const _withScopeId = n => (_pushScopeId("data-v-7f0e16e0"),n=n(),_popScopeId(),n);
const _hoisted_1 = { class: "toy-shell" };
const _hoisted_2 = { class: "toy-hero" };
const _hoisted_3 = { class: "toy-copy" };
const _hoisted_4 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-badge" }, "SQ玩偶", -1));
const _hoisted_5 = { class: "toy-title" };
const _hoisted_6 = { class: "toy-subtitle" };
const _hoisted_7 = { class: "toy-hero-meta" };
const _hoisted_8 = { class: "toy-actions" };
const _hoisted_9 = { class: "toy-overview-grid" };
const _hoisted_10 = { class: "toy-overview-label" };
const _hoisted_11 = { class: "toy-overview-value" };
const _hoisted_12 = {
  key: 0,
  class: "toy-overview-desc"
};
const _hoisted_13 = {
  key: 1,
  class: "toy-overview-desc"
};
const _hoisted_14 = {
  key: 1,
  class: "toy-panel"
};
const _hoisted_15 = { class: "toy-panel-head" };
const _hoisted_16 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", null, [
  /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "本次摘要"),
  /*#__PURE__*/_createElementVNode("h2", null, "任务结果")
], -1));
const _hoisted_17 = { class: "toy-summary-list" };
const _hoisted_18 = { class: "toy-panel" };
const _hoisted_19 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-head" }, [
  /*#__PURE__*/_createElementVNode("div", null, [
    /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "盲盒商店"),
    /*#__PURE__*/_createElementVNode("h2", null, "玩偶盲盒商店")
  ])
], -1));
const _hoisted_20 = { class: "toy-box-grid" };
const _hoisted_21 = ["src", "alt"];
const _hoisted_22 = { class: "toy-box-name" };
const _hoisted_23 = { class: "toy-box-desc" };
const _hoisted_24 = {
  key: 1,
  class: "toy-box-lock"
};
const _hoisted_25 = { class: "toy-box-actions" };
const _hoisted_26 = ["onUpdate:modelValue"];
const _hoisted_27 = { class: "toy-panel" };
const _hoisted_28 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-head" }, [
  /*#__PURE__*/_createElementVNode("div", null, [
    /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "我的盲盒"),
    /*#__PURE__*/_createElementVNode("h2", null, "我的盲盒")
  ])
], -1));
const _hoisted_29 = {
  key: 0,
  class: "toy-empty"
};
const _hoisted_30 = {
  key: 1,
  class: "toy-box-grid"
};
const _hoisted_31 = ["src", "alt"];
const _hoisted_32 = { class: "toy-box-name" };
const _hoisted_33 = { class: "toy-box-desc" };
const _hoisted_34 = { class: "toy-box-actions" };
const _hoisted_35 = ["onUpdate:modelValue", "max"];
const _hoisted_36 = { class: "toy-panel" };
const _hoisted_37 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-head" }, [
  /*#__PURE__*/_createElementVNode("div", null, [
    /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "玩偶柜子"),
    /*#__PURE__*/_createElementVNode("h2", null, "玩偶柜子")
  ])
], -1));
const _hoisted_38 = { class: "toy-toolbar" };
const _hoisted_39 = { class: "toy-sort-group" };
const _hoisted_40 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("span", null, "按级别", -1));
const _hoisted_41 = { class: "toy-sort-group" };
const _hoisted_42 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("span", null, "按可用数量", -1));
const _hoisted_43 = { class: "toy-sort-group" };
const _hoisted_44 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("span", null, "按最快冷却", -1));
const _hoisted_45 = { class: "toy-selected-bar" };
const _hoisted_46 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("span", null, "当前选中：", -1));
const _hoisted_47 = { key: 0 };
const _hoisted_48 = {
  key: 0,
  class: "toy-empty"
};
const _hoisted_49 = {
  key: 1,
  class: "toy-cabinet-grid"
};
const _hoisted_50 = ["onClick"];
const _hoisted_51 = ["src", "alt"];
const _hoisted_52 = {
  key: 1,
  class: "toy-doll-placeholder"
};
const _hoisted_53 = { class: "toy-doll-quality" };
const _hoisted_54 = { class: "toy-doll-name" };
const _hoisted_55 = { class: "toy-doll-meta" };
const _hoisted_56 = { class: "toy-doll-meta" };
const _hoisted_57 = { class: "toy-doll-meta" };
const _hoisted_58 = { class: "toy-doll-count" };
const _hoisted_59 = { class: "toy-doll-count" };
const _hoisted_60 = {
  key: 2,
  class: "toy-doll-cooldown"
};
const _hoisted_61 = { class: "toy-panel" };
const _hoisted_62 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-head" }, [
  /*#__PURE__*/_createElementVNode("div", null, [
    /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "我的展柜"),
    /*#__PURE__*/_createElementVNode("h2", null, "我的展柜")
  ])
], -1));
const _hoisted_63 = { class: "toy-slot-grid" };
const _hoisted_64 = { class: "toy-slot-index" };
const _hoisted_65 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-slot-empty" }, "空展位", -1));
const _hoisted_66 = { class: "toy-slot-tip" };
const _hoisted_67 = ["src", "alt"];
const _hoisted_68 = { class: "toy-slot-name" };
const _hoisted_69 = { class: "toy-slot-meta" };
const _hoisted_70 = { class: "toy-slot-meta" };
const _hoisted_71 = { class: "toy-slot-progress" };
const _hoisted_72 = { class: "toy-panel" };
const _hoisted_73 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-head" }, [
  /*#__PURE__*/_createElementVNode("div", null, [
    /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "外展抢位"),
    /*#__PURE__*/_createElementVNode("h2", null, "抢占他人展位")
  ])
], -1));
const _hoisted_74 = { class: "toy-target-controls" };
const _hoisted_75 = { class: "toy-search" };
const _hoisted_76 = {
  key: 0,
  class: "toy-empty"
};
const _hoisted_77 = {
  key: 1,
  class: "toy-target-panel"
};
const _hoisted_78 = { class: "toy-target-head" };
const _hoisted_79 = { class: "toy-slot-grid" };
const _hoisted_80 = { class: "toy-slot-index" };
const _hoisted_81 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-slot-empty" }, "空位可抢", -1));
const _hoisted_82 = { class: "toy-slot-tip" };
const _hoisted_83 = { class: "toy-slot-name" };
const _hoisted_84 = { class: "toy-slot-meta" };
const _hoisted_85 = { class: "toy-slot-meta" };
const _hoisted_86 = { class: "toy-panel" };
const _hoisted_87 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-head" }, [
  /*#__PURE__*/_createElementVNode("div", null, [
    /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "外展记录"),
    /*#__PURE__*/_createElementVNode("h2", null, "我的外展记录")
  ])
], -1));
const _hoisted_88 = {
  key: 0,
  class: "toy-empty"
};
const _hoisted_89 = {
  key: 1,
  class: "toy-remote-grid"
};
const _hoisted_90 = ["src", "alt"];
const _hoisted_91 = { class: "toy-remote-owner" };
const _hoisted_92 = { class: "toy-remote-meta" };
const _hoisted_93 = { class: "toy-remote-meta" };
const _hoisted_94 = { class: "toy-remote-meta" };
const _hoisted_95 = { class: "toy-panel" };
const _hoisted_96 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-head" }, [
  /*#__PURE__*/_createElementVNode("div", null, [
    /*#__PURE__*/_createElementVNode("div", { class: "toy-panel-kicker" }, "执行历史"),
    /*#__PURE__*/_createElementVNode("h2", null, "最新操作记录")
  ])
], -1));
const _hoisted_97 = {
  key: 0,
  class: "toy-empty"
};
const _hoisted_98 = {
  key: 1,
  class: "toy-history-list"
};
const _hoisted_99 = { class: "toy-history-message" };
const _hoisted_100 = { class: "toy-history-time" };

const {computed,onBeforeUnmount,onMounted,reactive,ref,watch} = await importShared('vue');


const pluginBase = '/plugin/SQToy';

const _sfc_main = {
  __name: 'Page',
  props: {
  api: { type: Object, required: true },
  initialConfig: { type: Object, default: () => ({}) },
},
  emits: ['switch', 'close'],
  setup(__props, { emit }) {

const props = __props;





const loading = ref(false);
const rootEl = ref(null);
const status = reactive({ toy_status: {}, history: [] });
const message = reactive({ text: '', type: 'success' });
const targetKeyword = ref('');
const selectedDollKey = ref('');
const sortField = ref('cooling_count');
const sortDirection = ref('asc');
const buyQuantities = reactive({});
const openQuantities = reactive({});
const isDarkTheme = ref(false);
const dismissedSummaryKey = ref('');

let themeObserver = null;
let mediaQuery = null;
let observedThemeNode = null;

const toy = computed(() => status.toy_status || {});
const overview = computed(() => toy.value.overview || []);
const shopBoxes = computed(() => toy.value.shop_boxes || []);
const myBoxes = computed(() => toy.value.my_boxes || []);
const personalSlots = computed(() => toy.value.personal_slots || []);
const targetPanel = computed(() => toy.value.target_panel || {});
const remoteRecords = computed(() => toy.value.remote_records || []);
const historyLogs = computed(() => toy.value.history_logs || []);
const summaryLines = computed(() => (toy.value.summary || []).filter(Boolean));
const summaryKey = computed(() => summaryLines.value.join('||'));
const showSummary = computed(() => !!summaryLines.value.length && dismissedSummaryKey.value !== summaryKey.value);

const cabinetCards = computed(() => {
  const items = [...(toy.value.cabinet || [])];
  return items.sort((left, right) => {
    const field = sortField.value;
    const lv = Number(left[field] || 0);
    const rv = Number(right[field] || 0);
    if (lv === rv) return String(left.name || '').localeCompare(String(right.name || ''))
    return sortDirection.value === 'desc' ? rv - lv : lv - rv
  })
});

const selectedDoll = computed(() => cabinetCards.value.find((item) => item.doll_key === selectedDollKey.value) || null);

watch(
  shopBoxes,
  (items) => {
    items.forEach((item) => {
      if (!buyQuantities[item.box_key]) buyQuantities[item.box_key] = String(item.default_quantity || 1);
    });
  },
  { immediate: true },
);

watch(
  myBoxes,
  (items) => {
    items.forEach((item) => {
      const key = item.box_key || item.name;
      if (!openQuantities[key]) openQuantities[key] = String(item.default_quantity || 1);
    });
  },
  { immediate: true },
);

function flash(text, type = 'success') {
  message.text = text;
  message.type = type;
}

function applyPayload(payload = {}) {
  if (payload.status && payload.status.toy_status) {
    Object.assign(status, payload.status);
  } else if (payload.toy_status || payload.history || payload.config) {
    Object.assign(status, payload);
  }
  if (payload.toy_status && !payload.status) {
    status.toy_status = payload.toy_status;
  }
  if (payload.target_panel && status.toy_status) {
    status.toy_status.target_panel = payload.target_panel;
  }
}

async function loadStatus() {
  const data = await props.api.get(`${pluginBase}/status`);
  Object.assign(status, data || {});
}

async function withAction(action, fallback) {
  loading.value = true;
  try {
    const result = await action();
    applyPayload(result || {});
    flash(result?.message || fallback);
  } catch (error) {
    flash(error?.message || fallback, 'error');
  } finally {
    loading.value = false;
  }
}

function refreshData() {
  return withAction(() => props.api.post(`${pluginBase}/refresh`), '状态已刷新')
}

function runNow() {
  return withAction(() => props.api.post(`${pluginBase}/run`), '执行完成')
}

function syncCookie() {
  return withAction(() => props.api.get(`${pluginBase}/cookie`), 'Cookie 已同步')
}

function buyBox(box) {
  return withAction(
    () =>
      props.api.post(`${pluginBase}/buy-box`, {
        box_key: box.box_key,
        quantity: Number(buyQuantities[box.box_key] || 1),
      }),
    '购买成功',
  )
}

function openBox(box) {
  const key = box.box_key || box.name;
  return withAction(
    () =>
      props.api.post(`${pluginBase}/open-box`, {
        box_key: box.box_key,
        quantity: Number(openQuantities[key] || 1),
      }),
    '开启完成',
  )
}

function selectDoll(doll) {
  if (!doll.can_place) return
  selectedDollKey.value = selectedDollKey.value === doll.doll_key ? '' : doll.doll_key;
}

function collectSlot(slot) {
  return withAction(
    () =>
      props.api.post(`${pluginBase}/collect-slot`, {
        owner_id: slot.owner_id,
        slot_index: slot.slot_index,
      }),
    '收回成功',
  )
}

function placePersonal(slot) {
  if (!selectedDoll.value) return
  return withAction(
    () =>
      props.api.post(`${pluginBase}/place-personal`, {
        owner_id: slot.owner_id,
        slot_index: slot.slot_index,
        doll_key: selectedDoll.value.doll_key,
        doll_name: selectedDoll.value.name,
      }),
    '上架成功',
  )
}

function randomTarget() {
  return withAction(() => props.api.post(`${pluginBase}/random-target`), '已匹配目标')
}

function viewTarget(ownerId = null) {
  const keyword = ownerId ?? targetKeyword.value;
  return withAction(() => props.api.post(`${pluginBase}/view-target`, { keyword }), '已加载目标展台')
}

function placeTarget(slot) {
  if (!selectedDoll.value) return
  return withAction(
    () =>
      props.api.post(`${pluginBase}/place-target`, {
        owner_id: slot.owner_id,
        slot_index: slot.slot_index,
        doll_key: selectedDoll.value.doll_key,
        doll_name: selectedDoll.value.name,
      }),
    '抢占成功',
  )
}

function setSort(field, direction) {
  sortField.value = field;
  sortDirection.value = direction;
}

function dismissSummary() {
  dismissedSummaryKey.value = summaryKey.value;
  sessionStorage.setItem('sqtoy-summary-dismissed', dismissedSummaryKey.value);
}

function closePlugin() {
  emit('close');
}

function findThemeNode() {
  let current = rootEl.value;
  while (current) {
    if (current.getAttribute?.('data-theme')) return current
    current = current.parentElement;
  }
  if (document.body?.getAttribute('data-theme')) return document.body
  if (document.documentElement?.getAttribute('data-theme')) return document.documentElement
  return null
}

function detectTheme() {
  const themeNode = findThemeNode();
  const themeValue = themeNode?.getAttribute?.('data-theme') || '';
  const darkThemes = new Set(['dark', 'purple', 'transparent']);
  if (darkThemes.has(themeValue)) {
    isDarkTheme.value = true;
    return
  }
  isDarkTheme.value = !!window.matchMedia?.('(prefers-color-scheme: dark)').matches;
}

function bindTheme() {
  detectTheme();
  observedThemeNode = findThemeNode();
  if (observedThemeNode && window.MutationObserver) {
    themeObserver = new MutationObserver(detectTheme);
    themeObserver.observe(observedThemeNode, { attributes: true, attributeFilter: ['data-theme'] });
  }
  if (window.matchMedia) {
    mediaQuery = window.matchMedia('(prefers-color-scheme: dark)');
    mediaQuery.addEventListener?.('change', detectTheme);
  }
}

onMounted(async () => {
  dismissedSummaryKey.value = sessionStorage.getItem('sqtoy-summary-dismissed') || '';
  bindTheme();
  await loadStatus();
});

onBeforeUnmount(() => {
  themeObserver?.disconnect?.();
  mediaQuery?.removeEventListener?.('change', detectTheme);
});

return (_ctx, _cache) => {
  const _component_v_btn = _resolveComponent("v-btn");
  const _component_v_alert = _resolveComponent("v-alert");

  return (_openBlock(), _createElementBlock("div", {
    ref_key: "rootEl",
    ref: rootEl,
    class: _normalizeClass(["toy-page", { 'is-dark-theme': isDarkTheme.value }])
  }, [
    _createElementVNode("div", _hoisted_1, [
      _createElementVNode("section", _hoisted_2, [
        _createElementVNode("div", _hoisted_3, [
          _hoisted_4,
          _createElementVNode("h1", _hoisted_5, _toDisplayString(toy.value.title || '玩偶抢曝光'), 1),
          _createElementVNode("p", _hoisted_6, _toDisplayString(toy.value.subtitle || '盲盒抽玩偶、自动回收、个人展位与外展抢位。'), 1),
          _createElementVNode("div", _hoisted_7, [
            _createElementVNode("span", null, "最近执行 " + _toDisplayString(status.last_run || '暂无'), 1),
            _createElementVNode("span", null, "下次运行 " + _toDisplayString(toy.value.next_run_time || '等待刷新'), 1),
            _createElementVNode("span", null, "Cookie " + _toDisplayString(toy.value.cookie_source || status.cookie_source || '未同步'), 1)
          ])
        ]),
        _createElementVNode("div", _hoisted_8, [
          _createVNode(_component_v_btn, {
            color: "success",
            variant: "flat",
            loading: loading.value,
            onClick: runNow
          }, {
            default: _withCtx(() => [
              _createTextVNode("立即执行")
            ]),
            _: 1
          }, 8, ["loading"]),
          _createVNode(_component_v_btn, {
            color: "primary",
            variant: "flat",
            loading: loading.value,
            onClick: refreshData
          }, {
            default: _withCtx(() => [
              _createTextVNode("刷新状态")
            ]),
            _: 1
          }, 8, ["loading"]),
          _createVNode(_component_v_btn, {
            color: "warning",
            variant: "flat",
            loading: loading.value,
            onClick: syncCookie
          }, {
            default: _withCtx(() => [
              _createTextVNode("同步 Cookie")
            ]),
            _: 1
          }, 8, ["loading"]),
          _createVNode(_component_v_btn, {
            variant: "text",
            onClick: _cache[0] || (_cache[0] = $event => (emit('switch', 'config')))
          }, {
            default: _withCtx(() => [
              _createTextVNode("配置")
            ]),
            _: 1
          }),
          _createVNode(_component_v_btn, {
            variant: "text",
            onClick: closePlugin
          }, {
            default: _withCtx(() => [
              _createTextVNode("关闭")
            ]),
            _: 1
          })
        ])
      ]),
      (message.text)
        ? (_openBlock(), _createBlock(_component_v_alert, {
            key: 0,
            type: message.type,
            variant: "tonal",
            class: "mb-4"
          }, {
            default: _withCtx(() => [
              _createTextVNode(_toDisplayString(message.text), 1)
            ]),
            _: 1
          }, 8, ["type"]))
        : _createCommentVNode("", true),
      _createElementVNode("section", _hoisted_9, [
        (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(overview.value, (item) => {
          return (_openBlock(), _createElementBlock("article", {
            key: item.label,
            class: "toy-overview-card"
          }, [
            _createElementVNode("div", _hoisted_10, _toDisplayString(item.label), 1),
            _createElementVNode("div", _hoisted_11, _toDisplayString(item.value), 1),
            (item.desc)
              ? (_openBlock(), _createElementBlock("div", _hoisted_12, _toDisplayString(item.desc), 1))
              : _createCommentVNode("", true),
            (item.extra)
              ? (_openBlock(), _createElementBlock("div", _hoisted_13, _toDisplayString(item.extra), 1))
              : _createCommentVNode("", true)
          ]))
        }), 128))
      ]),
      (showSummary.value)
        ? (_openBlock(), _createElementBlock("section", _hoisted_14, [
            _createElementVNode("div", _hoisted_15, [
              _hoisted_16,
              _createVNode(_component_v_btn, {
                variant: "text",
                size: "small",
                onClick: dismissSummary
              }, {
                default: _withCtx(() => [
                  _createTextVNode("关闭")
                ]),
                _: 1
              })
            ]),
            _createElementVNode("div", _hoisted_17, [
              (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(summaryLines.value, (line) => {
                return (_openBlock(), _createElementBlock("div", {
                  key: line,
                  class: "toy-summary-line"
                }, _toDisplayString(line), 1))
              }), 128))
            ])
          ]))
        : _createCommentVNode("", true),
      _createElementVNode("section", _hoisted_18, [
        _hoisted_19,
        _createElementVNode("div", _hoisted_20, [
          (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(shopBoxes.value, (box) => {
            return (_openBlock(), _createElementBlock("article", {
              key: box.box_key || box.name,
              class: _normalizeClass(["toy-box-card", { locked: box.locked }])
            }, [
              (box.image)
                ? (_openBlock(), _createElementBlock("img", {
                    key: 0,
                    class: "toy-box-image",
                    src: box.image,
                    alt: box.name
                  }, null, 8, _hoisted_21))
                : _createCommentVNode("", true),
              _createElementVNode("div", _hoisted_22, _toDisplayString(box.name), 1),
              _createElementVNode("div", _hoisted_23, _toDisplayString(box.desc), 1),
              (box.lock_text)
                ? (_openBlock(), _createElementBlock("div", _hoisted_24, _toDisplayString(box.lock_text), 1))
                : _createCommentVNode("", true),
              _createElementVNode("div", _hoisted_25, [
                _withDirectives(_createElementVNode("input", {
                  "onUpdate:modelValue": $event => ((buyQuantities[box.box_key]) = $event),
                  class: "toy-number-input",
                  type: "number",
                  min: "1"
                }, null, 8, _hoisted_26), [
                  [_vModelText, buyQuantities[box.box_key]]
                ]),
                _createVNode(_component_v_btn, {
                  color: "deep-orange",
                  variant: "flat",
                  loading: loading.value,
                  disabled: loading.value || box.buy_enabled === false,
                  onClick: $event => (buyBox(box))
                }, {
                  default: _withCtx(() => [
                    _createTextVNode(" 购买 ")
                  ]),
                  _: 2
                }, 1032, ["loading", "disabled", "onClick"])
              ])
            ], 2))
          }), 128))
        ])
      ]),
      _createElementVNode("section", _hoisted_27, [
        _hoisted_28,
        (!myBoxes.value.length)
          ? (_openBlock(), _createElementBlock("div", _hoisted_29, "暂无盲盒，去商店看看吧"))
          : (_openBlock(), _createElementBlock("div", _hoisted_30, [
              (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(myBoxes.value, (box) => {
                return (_openBlock(), _createElementBlock("article", {
                  key: `owned-${box.box_key || box.name}`,
                  class: "toy-box-card compact"
                }, [
                  (box.image)
                    ? (_openBlock(), _createElementBlock("img", {
                        key: 0,
                        class: "toy-box-image small",
                        src: box.image,
                        alt: box.name
                      }, null, 8, _hoisted_31))
                    : _createCommentVNode("", true),
                  _createElementVNode("div", _hoisted_32, _toDisplayString(box.name), 1),
                  _createElementVNode("div", _hoisted_33, "拥有 " + _toDisplayString(box.count) + " 个", 1),
                  _createElementVNode("div", _hoisted_34, [
                    _withDirectives(_createElementVNode("input", {
                      "onUpdate:modelValue": $event => ((openQuantities[box.box_key || box.name]) = $event),
                      class: "toy-number-input",
                      type: "number",
                      min: "1",
                      max: Math.max(Number(box.count || 0), 1)
                    }, null, 8, _hoisted_35), [
                      [_vModelText, openQuantities[box.box_key || box.name]]
                    ]),
                    _createVNode(_component_v_btn, {
                      color: "pink",
                      variant: "flat",
                      loading: loading.value,
                      disabled: loading.value || !Number(box.count || 0),
                      onClick: $event => (openBox(box))
                    }, {
                      default: _withCtx(() => [
                        _createTextVNode("开启")
                      ]),
                      _: 2
                    }, 1032, ["loading", "disabled", "onClick"])
                  ])
                ]))
              }), 128))
            ]))
      ]),
      _createElementVNode("section", _hoisted_36, [
        _hoisted_37,
        _createElementVNode("div", _hoisted_38, [
          _createElementVNode("div", _hoisted_39, [
            _hoisted_40,
            _createElementVNode("button", {
              type: "button",
              class: "toy-chip",
              onClick: _cache[1] || (_cache[1] = $event => (setSort('quality_rank', 'desc')))
            }, "高→低"),
            _createElementVNode("button", {
              type: "button",
              class: "toy-chip",
              onClick: _cache[2] || (_cache[2] = $event => (setSort('quality_rank', 'asc')))
            }, "低→高")
          ]),
          _createElementVNode("div", _hoisted_41, [
            _hoisted_42,
            _createElementVNode("button", {
              type: "button",
              class: "toy-chip",
              onClick: _cache[3] || (_cache[3] = $event => (setSort('available', 'desc')))
            }, "多→少"),
            _createElementVNode("button", {
              type: "button",
              class: "toy-chip",
              onClick: _cache[4] || (_cache[4] = $event => (setSort('available', 'asc')))
            }, "少→多")
          ]),
          _createElementVNode("div", _hoisted_43, [
            _hoisted_44,
            _createElementVNode("button", {
              type: "button",
              class: "toy-chip",
              onClick: _cache[5] || (_cache[5] = $event => (setSort('cooling_count', 'asc')))
            }, "短→长"),
            _createElementVNode("button", {
              type: "button",
              class: "toy-chip",
              onClick: _cache[6] || (_cache[6] = $event => (setSort('cooling_count', 'desc')))
            }, "长→短")
          ])
        ]),
        _createElementVNode("div", _hoisted_45, [
          _hoisted_46,
          _createElementVNode("strong", null, _toDisplayString(selectedDoll.value ? selectedDoll.value.name : '未选择玩偶'), 1),
          (selectedDoll.value)
            ? (_openBlock(), _createElementBlock("span", _hoisted_47, "，点击空展位或目标空位即可上架"))
            : _createCommentVNode("", true)
        ]),
        (!cabinetCards.value.length)
          ? (_openBlock(), _createElementBlock("div", _hoisted_48, "暂无可用空位或玩偶，稍后再试"))
          : (_openBlock(), _createElementBlock("div", _hoisted_49, [
              (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(cabinetCards.value, (doll) => {
                return (_openBlock(), _createElementBlock("article", {
                  key: doll.doll_key || doll.name,
                  class: _normalizeClass(["toy-doll-card", { selected: selectedDollKey.value === doll.doll_key, disabled: !doll.can_place }]),
                  onClick: $event => (selectDoll(doll))
                }, [
                  (doll.image)
                    ? (_openBlock(), _createElementBlock("img", {
                        key: 0,
                        class: "toy-doll-image",
                        src: doll.image,
                        alt: doll.name
                      }, null, 8, _hoisted_51))
                    : (_openBlock(), _createElementBlock("div", _hoisted_52, "🧸")),
                  _createElementVNode("div", _hoisted_53, _toDisplayString(doll.quality || '未识别'), 1),
                  _createElementVNode("div", _hoisted_54, _toDisplayString(doll.name), 1),
                  _createElementVNode("div", _hoisted_55, _toDisplayString(doll.display_text), 1),
                  _createElementVNode("div", _hoisted_56, _toDisplayString(doll.reward_text), 1),
                  _createElementVNode("div", _hoisted_57, _toDisplayString(doll.origin), 1),
                  _createElementVNode("div", _hoisted_58, "可用/总数 " + _toDisplayString(doll.available) + " / " + _toDisplayString(doll.total), 1),
                  _createElementVNode("div", _hoisted_59, "展出" + _toDisplayString(doll.display_count) + " · 冷却" + _toDisplayString(doll.cooling_count), 1),
                  (doll.cooldown_text)
                    ? (_openBlock(), _createElementBlock("div", _hoisted_60, _toDisplayString(doll.cooldown_text), 1))
                    : _createCommentVNode("", true),
                  _createVNode(_component_v_btn, {
                    block: "",
                    size: "small",
                    color: "deep-orange",
                    variant: "flat",
                    disabled: !doll.can_place,
                    class: "mt-3"
                  }, {
                    default: _withCtx(() => [
                      _createTextVNode(_toDisplayString(selectedDollKey.value === doll.doll_key ? '已选择' : '选择上架'), 1)
                    ]),
                    _: 2
                  }, 1032, ["disabled"])
                ], 10, _hoisted_50))
              }), 128))
            ]))
      ]),
      _createElementVNode("section", _hoisted_61, [
        _hoisted_62,
        _createElementVNode("div", _hoisted_63, [
          (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(personalSlots.value, (slot) => {
            return (_openBlock(), _createElementBlock("article", {
              key: `personal-${slot.slot_index}`,
              class: _normalizeClass(["toy-slot-card", { empty: slot.empty }])
            }, [
              _createElementVNode("div", _hoisted_64, "展位 " + _toDisplayString(slot.slot_index), 1),
              (slot.empty)
                ? (_openBlock(), _createElementBlock(_Fragment, { key: 0 }, [
                    _hoisted_65,
                    _createElementVNode("div", _hoisted_66, _toDisplayString(selectedDoll.value ? `可上架 ${selectedDoll.value.name}` : '先从玩偶柜子选择玩偶'), 1),
                    _createVNode(_component_v_btn, {
                      color: "deep-orange",
                      variant: "flat",
                      disabled: !selectedDoll.value || loading.value,
                      onClick: $event => (placePersonal(slot))
                    }, {
                      default: _withCtx(() => [
                        _createTextVNode("上架所选玩偶")
                      ]),
                      _: 2
                    }, 1032, ["disabled", "onClick"])
                  ], 64))
                : (_openBlock(), _createElementBlock(_Fragment, { key: 1 }, [
                    (slot.image)
                      ? (_openBlock(), _createElementBlock("img", {
                          key: 0,
                          class: "toy-slot-image",
                          src: slot.image,
                          alt: slot.doll_name
                        }, null, 8, _hoisted_67))
                      : _createCommentVNode("", true),
                    _createElementVNode("div", _hoisted_68, _toDisplayString(slot.doll_name), 1),
                    _createElementVNode("div", _hoisted_69, _toDisplayString(slot.remaining_text), 1),
                    _createElementVNode("div", _hoisted_70, _toDisplayString(slot.reward_text), 1),
                    _createElementVNode("div", _hoisted_71, [
                      _createElementVNode("div", {
                        class: "toy-slot-progress-bar",
                        style: _normalizeStyle({ width: `${slot.progress}%` })
                      }, null, 4)
                    ]),
                    _createVNode(_component_v_btn, {
                      color: "primary",
                      variant: "flat",
                      loading: loading.value,
                      onClick: $event => (collectSlot(slot))
                    }, {
                      default: _withCtx(() => [
                        _createTextVNode(_toDisplayString(slot.can_collect ? '收回玩偶' : '尝试收回'), 1)
                      ]),
                      _: 2
                    }, 1032, ["loading", "onClick"])
                  ], 64))
            ], 2))
          }), 128))
        ])
      ]),
      _createElementVNode("section", _hoisted_72, [
        _hoisted_73,
        _createElementVNode("div", _hoisted_74, [
          _createVNode(_component_v_btn, {
            color: "deep-orange",
            variant: "flat",
            loading: loading.value,
            onClick: randomTarget
          }, {
            default: _withCtx(() => [
              _createTextVNode("随机匹配")
            ]),
            _: 1
          }, 8, ["loading"]),
          _createElementVNode("div", _hoisted_75, [
            _withDirectives(_createElementVNode("input", {
              "onUpdate:modelValue": _cache[7] || (_cache[7] = $event => ((targetKeyword).value = $event)),
              class: "toy-text-input",
              placeholder: "输入用户名或用户 ID"
            }, null, 512), [
              [_vModelText, targetKeyword.value]
            ]),
            _createVNode(_component_v_btn, {
              variant: "flat",
              color: "primary",
              loading: loading.value,
              onClick: _cache[8] || (_cache[8] = $event => (viewTarget()))
            }, {
              default: _withCtx(() => [
                _createTextVNode("前往展台")
              ]),
              _: 1
            }, 8, ["loading"])
          ])
        ]),
        (!targetPanel.value.slots?.length)
          ? (_openBlock(), _createElementBlock("div", _hoisted_76, "尚未选择目标"))
          : (_openBlock(), _createElementBlock("div", _hoisted_77, [
              _createElementVNode("div", _hoisted_78, _toDisplayString(targetPanel.value.username) + " · " + _toDisplayString(targetPanel.value.slot_count) + " 个展位", 1),
              _createElementVNode("div", _hoisted_79, [
                (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(targetPanel.value.slots, (slot) => {
                  return (_openBlock(), _createElementBlock("article", {
                    key: `target-${slot.owner_id}-${slot.slot_index}`,
                    class: _normalizeClass(["toy-slot-card", { empty: slot.empty }])
                  }, [
                    _createElementVNode("div", _hoisted_80, "展位 " + _toDisplayString(slot.slot_index), 1),
                    (slot.empty && !slot.cooldown_active)
                      ? (_openBlock(), _createElementBlock(_Fragment, { key: 0 }, [
                          _hoisted_81,
                          _createElementVNode("div", _hoisted_82, _toDisplayString(selectedDoll.value ? `抢占为 ${selectedDoll.value.name}` : '先选择玩偶'), 1),
                          _createVNode(_component_v_btn, {
                            color: "deep-orange",
                            variant: "flat",
                            disabled: !selectedDoll.value || loading.value,
                            onClick: $event => (placeTarget(slot))
                          }, {
                            default: _withCtx(() => [
                              _createTextVNode("抢占展位")
                            ]),
                            _: 2
                          }, 1032, ["disabled", "onClick"])
                        ], 64))
                      : (_openBlock(), _createElementBlock(_Fragment, { key: 1 }, [
                          _createElementVNode("div", _hoisted_83, _toDisplayString(slot.doll_name || slot.state_text), 1),
                          _createElementVNode("div", _hoisted_84, _toDisplayString(slot.remaining_text), 1),
                          _createElementVNode("div", _hoisted_85, _toDisplayString(slot.state_text), 1),
                          (slot.viewer_is_occupant)
                            ? (_openBlock(), _createBlock(_component_v_btn, {
                                key: 0,
                                color: "primary",
                                variant: "flat",
                                disabled: loading.value,
                                onClick: $event => (collectSlot(slot))
                              }, {
                                default: _withCtx(() => [
                                  _createTextVNode(_toDisplayString(slot.can_collect ? '收回玩偶' : '尝试收回'), 1)
                                ]),
                                _: 2
                              }, 1032, ["disabled", "onClick"]))
                            : _createCommentVNode("", true)
                        ], 64))
                  ], 2))
                }), 128))
              ])
            ]))
      ]),
      _createElementVNode("section", _hoisted_86, [
        _hoisted_87,
        (!remoteRecords.value.length)
          ? (_openBlock(), _createElementBlock("div", _hoisted_88, "暂无外展记录"))
          : (_openBlock(), _createElementBlock("div", _hoisted_89, [
              (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(remoteRecords.value, (item) => {
                return (_openBlock(), _createElementBlock("article", {
                  key: `${item.owner_id}-${item.slot_index}`,
                  class: "toy-remote-card"
                }, [
                  (item.image)
                    ? (_openBlock(), _createElementBlock("img", {
                        key: 0,
                        class: "toy-remote-image",
                        src: item.image,
                        alt: item.doll_name
                      }, null, 8, _hoisted_90))
                    : _createCommentVNode("", true),
                  _createElementVNode("div", _hoisted_91, _toDisplayString(item.owner_name), 1),
                  _createElementVNode("div", _hoisted_92, "展位 " + _toDisplayString(item.slot_index), 1),
                  _createElementVNode("div", _hoisted_93, _toDisplayString(item.doll_name), 1),
                  _createElementVNode("div", _hoisted_94, _toDisplayString(item.remaining_text), 1),
                  _createVNode(_component_v_btn, {
                    size: "small",
                    variant: "flat",
                    color: "primary",
                    disabled: loading.value,
                    onClick: $event => (viewTarget(item.owner_id))
                  }, {
                    default: _withCtx(() => [
                      _createTextVNode("查看")
                    ]),
                    _: 2
                  }, 1032, ["disabled", "onClick"])
                ]))
              }), 128))
            ]))
      ]),
      _createElementVNode("section", _hoisted_95, [
        _hoisted_96,
        (!historyLogs.value.length)
          ? (_openBlock(), _createElementBlock("div", _hoisted_97, "暂无最近操作记录"))
          : (_openBlock(), _createElementBlock("div", _hoisted_98, [
              (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(historyLogs.value, (item) => {
                return (_openBlock(), _createElementBlock("article", {
                  key: `${item.time}-${item.message}`,
                  class: "toy-history-item"
                }, [
                  _createElementVNode("div", _hoisted_99, _toDisplayString(item.message), 1),
                  _createElementVNode("div", _hoisted_100, _toDisplayString(item.time), 1)
                ]))
              }), 128))
            ]))
      ])
    ])
  ], 2))
}
}

};
const PageView = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-7f0e16e0"]]);

export { PageView as default };
