...
class SQFarm2(_PluginBase):
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [
            {"path": "/config", "endpoint": self._get_config, "methods": ["GET"], "auth": "bear", "summary": "获取 SQ农场2配置"},
            {"path": "/config", "endpoint": self._save_config, "methods": ["POST"], "auth": "bear", "summary": "保存 SQ农场2配置"},
            {"path": "/status", "endpoint": self._get_status, "methods": ["GET"], "auth": "bear", "summary": "获取 SQ农场2状态"},
            {"path": "/refresh", "endpoint": self._refresh_data, "methods": ["POST"], "auth": "bear", "summary": "刷新农场数据"},
            {"path": "/run", "endpoint": self._run_now, "methods": ["POST"], "auth": "bear", "summary": "立即执行一次 SQ农场2"},
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._enabled:
            ...
            if next_run:
                ...
                services.append({
                    "id": "SQFarm2_auto",
                    "name": "SQ农场2初始化" if self._bootstrap_pending else "SQ农场2智能调度",
                    "trigger": "date",
                    "func": job_func,
    ...
    def run_job(self, force: bool = False, reason: str = "manual") -> Dict[str, Any]:
        ...
        try:
            ...
            if self._notify and (has_action_lines or has_warning_lines):
                if has_action_lines:
                    notify_title = "【🌱SQ农场2】 任务报告"
                    notify_text = self._build_notify_text(msg_lines)
                else:
                    notify_title = "【⚠️SQ农场2】 收菜失败"
                    notify_text = "\n".join(msg_lines)
